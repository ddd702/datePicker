...
(function ($) {   
    ...
    $.fn.datePicker = function (options) {   
        ...
        $.fn.datePicker.defaultOptions = {
            ...
            show:true,
            scrollOpt:{snap:"li",checkDOMChanges:true},
            callBack:function(){}
        }  
        ...
        function init(){//初始化函数
            ...
            if (!opts.curdate&&$.trim(initVal)!='') {
                var inputDate=null,inputTime=null;
        ...
        function showTxt(){
            var y=$('#yearScroll li').eq(initY).data('num'),
                ...
                date=new Date(y+'-'+m+'-'+d);
            switch(opts.theme){
                case 'date':$('.d-day-info').html(opts.days[date.getDay()]+"&nbsp;");$('.d-return-info').html(y+'-'+m+'-'+d);break;
                case 'datetime':$('.d-day-info').html(opts.days[date.getDay()]+"&nbsp;");$('.d-return-info').html(y+'-'+m+'-'+d+' '+h+':'+i);break;
                case 'time':$('.d-return-info').html(h+':'+i);break;
                case 'month':$('.d-return-info').html(y+'-'+m);break;
                default:$('.d-day-info').html(opts.days[date.getDay()]+"&nbsp;");$('.d-return-info').html(y+'-'+m+'-'+d);break;
            } 
        }
        function createYear(){
            ...
            yearScroll.on('scrollEnd', function(event) {
                var yIndex=this.currentPage.pageY;
                initY=yIndex+1;
                ...
                if(initM==2&&opts.theme!='month'){createDay(opts.monthDay[initM-1]);}
                showTxt();
            });
        }
        ...
        function createHour(){
            ...
            hourScroll.on('scrollEnd', function(event) {
                initH=this.currentPage.pageY+1;
                showTxt();
            });
        }
        function createMinute(){
            ...
            minuteScroll.on('scrollEnd', function(event) {
                initI=this.currentPage.pageY+1;
                showTxt();
            });
        }
    }
